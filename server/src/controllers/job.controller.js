...
const queryNearbyWorkers = async ({ location, category, radiusKm }) => {
    ...
    const radiusKmCapped = Math.min(
        Number(radiusKm || PLATFORM_CONFIG.defaultSearchRadiusKm),
        10
    );
...
export const createJob = async (req, res) => {
    try {
        ...
        const searchRadiusKm = Math.min(
            Number(req.body.searchRadiusKm || req.user.workerProfile?.serviceRadiusKm || PLATFORM_CONFIG.defaultSearchRadiusKm),
            10
        );
...
export const selectWorker = async (req, res) => {
    try {
        ...
        await applyWalletDebit({
            user: worker,
            amount: PLATFORM_CONFIG.securityDeposit || 50,
            type: "security_deposit",
            description: `Security deposit held for job "${job.title}"`,
            jobId: job._id,
        });
...
export const cancelJob = async (req, res) => {
    ...
    await handleSecurityDepositOnCancellation(job);
...
export const confirmJobCompletion = async (req, res) => {
    ...
    if (worker) {
        await applyWalletCredit({
            ...
            description: `Security deposit returned for job "${job.title}"`,
            jobId: job._id,
        });
    }
