...
export const handleSecurityDepositOnCancellation = async (job) => {
    if (!job.selectedWorker) {
        return;
    }

    const securityAmount = PLATFORM_CONFIG.securityDeposit || 50;
    const User = (await import("../models/user.model.js")).default;
    
    // Check if worker arrived
    if (job.timeline?.workerArrivedAt) {
        // Refund worker
        ...
        if (worker) {
            await applyWalletCredit({
                ...
                description: `Security deposit refunded for "${job.title}" after cancellation`,
                jobId: job._id,
            });
        }
    } else {
        ...
        if (customer) {
            await applyWalletCredit({
                ...
                description: `Security deposit paid to you because worker did not arrive for "${job.title}"`,
                jobId: job._id,
            });
        }
    }
